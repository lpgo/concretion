	...
	<div style="padding:20px;">
		<div class="noprint">
			...
			<div style="display:flex">
				...
				<mu-auto-complete :filter="myfilter" hintText="请输入车号" v-model="form.car" openOnFocus :dataSource="carPlates" :dataSourceConfig="{text:'_id',value:'_id'}" :maxSearchResults="10" :errorText="error" @change="error = ''"/>
				<!--
		...
		<div class="myDivToPrint">
			...
			<table border="1" bordercolor="black" cellspacing="0" cellpadding="5" width="100%" >
	    		...
	    		<tr v-for="(value,index) in data">
	    			<td>{{index+1}}</td>
	    			<td>{{value._id.com}}</td>
	    			<td>{{value._id.project}}</td>
	    			<td>{{value._id.distance}}</td>
